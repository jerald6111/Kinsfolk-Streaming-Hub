...
const Navigation = ({ currentView, setCurrentView, tmdbApiKey, youtubeApiKey, loadRealContent, loading, searchQuery, setSearchQuery, setShowSettings }) => (
...
const VideoPlayer = ({ currentVideo, videoRef, handleTimeUpdate, setDuration, setIsPlaying, showControls, handleSeek, currentTime, duration, togglePlayPause, isPlaying, volume, setVolume, setCurrentVideo, setCurrentView }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
            <video ref={videoRef} src={currentVideo.url} className="w-full h-full object-contain" onTimeUpdate={handleTimeUpdate} onLoadedMetadata={() => setDuration(videoRef.current.duration)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} autoPlay />
            {showControls && (
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black to-transparent p-8">
                <div className="max-w-4xl mx-auto">
                    <div className="mb-4"><div className="w-full h-2 bg-gray-600 rounded-full cursor-pointer" onClick={handleSeek}><div className="h-full bg-white rounded-full transition-all" style={{ width: `${(currentTime / duration) * 100}%` }} /></div></div>
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button onClick={() => { if (videoRef.current.requestFullscreen) { videoRef.current.requestFullscreen(); } }} className="p-2 hover:bg-white hover:bg-opacity-20 rounded-full"><Maximize className="w-6 h-6" /></button>
...
const StatsView = ({ viewingStats, continueWatching, localFiles, getAllContent }) => (
    <div className="pt-24 pb-12 px-6 max-w-7xl mx-auto">
      <h2 className="text-3xl font-bold text-white mb-8">Viewing Statistics</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700"><div className="flex items-center space-x-3 mb-4"><Clock className="w-8 h-8 text-blue-400" /><h3 className="text-xl font-semibold text-white">Watch Time</h3></div><p className="text-3xl font-bold text-blue-400">{Math.round(viewingStats.totalHours)} hrs</p></div>
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700"><div className="flex items-center space-x-3 mb-4"><TrendingUp className="w-8 h-8 text-green-400" /><h3 className="text-xl font-semibold text-white">Continue Watching</h3></div><p className="text-3xl font-bold text-green-400">{continueWatching.length}</p></div>
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700"><div className="flex items-center space-x-3 mb-4"><Grid className="w-8 h-8 text-purple-400" /><h3 className="text-xl font-semibold text-white">Local Files</h3></div><p className="text-3xl font-bold text-purple-400">{localFiles.length}</p></div>
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700"><div className="flex items-center space-x-3 mb-4"><Star className="w-8 h-8 text-yellow-400" /><h3 className="text-xl font-semibold text-white">Total Content</h3></div><p className="text-3xl font-bold text-yellow-400">{getAllContent().length}</p></div>
      </div>
      {viewingStats.recentWatches.length > 0 && <div className="bg-gray-800 rounded-xl p-6 border border-gray-700"><h3 className="text-xl font-semibold text-white mb-4">Recent Watches</h3><div className="space-y-3">{viewingStats.recentWatches.slice(0, 10).map((watch, index) => <div key={index} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-b-0"><div><p className="text-white font-medium">{watch.title}</p><p className="text-gray-400 text-sm capitalize">{watch.platform}</p></div><p className="text-gray-500 text-sm">{new Date(watch.watchedAt).toLocaleDateString()}</p></div>)}</div></div>}
    </div>
);

const HomeView = ({ tmdbApiKey, youtubeApiKey, setShowSettings, handleSelectLocalFolder, continueWatching, realContent, selectedPlatforms, searchQuery, localFiles, playVideo, platformColors, getAllContent }) => {
        ...
        <div className="pt-24 pb-12">
            {!tmdbApiKey && !youtubeApiKey && (
                ...
                <div className="flex space-x-4">
                    <a href="https://www.themoviedb.org/settings/api" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">Get TMDB Key (Free)</a>
                    <a href="https://console.developers.google.com/" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm">Get YouTube Key (Free)</a>
                    <button onClick={() => setShowSettings(true)} className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors text-sm">Open Settings</button>
                </div>
                </div>
            )}
            {continueWatching.length > 0 && (
                <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><Clock className="w-8 h-8 mr-3 text-blue-400" />Continue Watching</h2><div className="flex space-x-6 overflow-x-auto pb-4">{continueWatching.map(content => <ContentTile key={`continue-${content.id}`} content={content} size="large" playVideo={playVideo} platformColors={platformColors} />)}</div></section>
            )}
            {realContent.trending.length > 0 && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><TrendingUp className="w-8 h-8 mr-3 text-red-400" />Trending Now</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{realContent.trending.filter(c => selectedPlatforms[c.platform] && (searchQuery === '' || c.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div></section>}
            {realContent.newReleases.length > 0 && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><Calendar className="w-8 h-8 mr-3 text-green-400" />Popular Movies</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{realContent.newReleases.filter(c => selectedPlatforms[c.platform] && (searchQuery === '' || c.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div></section>}
            {realContent.top10.length > 0 && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><Star className="w-8 h-8 mr-3 text-yellow-400" />Top Rated TV Shows</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{realContent.top10.filter(c => selectedPlatforms[c.platform] && (searchQuery === '' || c.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div></section>}
            {realContent.youtube.length > 0 && selectedPlatforms.youtube && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><Play className="w-8 h-8 mr-3 text-red-500" />YouTube Highlights</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{realContent.youtube.filter(c => (searchQuery === '' || c.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div></section>}
            {localFiles.length > 0 && selectedPlatforms.local && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><FolderOpen className="w-8 h-8 mr-3 text-purple-400" />Local Files ({localFiles.length})</h2><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{localFiles.filter(c => (searchQuery === '' || c.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div></section>}
            {searchQuery && <section className="mb-12 px-6"><h2 className="text-3xl font-bold text-white mb-6 flex items-center"><Search className="w-8 h-8 mr-3 text-blue-400" />Search Results for "{searchQuery}" ({allContent.length})</h2>{allContent.length > 0 ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">{allContent.map(content => <ContentTile key={content.id} content={content} playVideo={playVideo} platformColors={platformColors} />)}</div> : <div className="text-center py-12"><Search className="w-16 h-16 text-gray-600 mx-auto mb-4" /><p className="text-gray-400 text-lg">No results found for "{searchQuery}"</p><p className="text-gray-500 text-sm mt-2">Try a different search term or load content from APIs</p></div>}</section>}
            {!hasContent && !searchQuery && <div className="text-center py-20 px-6"><div className="max-w-md mx-auto"><TrendingUp className="w-20 h-20 text-gray-600 mx-auto mb-6" /><h3 className="text-2xl font-bold text-white mb-4">Welcome to Kinsfolk Streaming Hub!</h3><p className="text-gray-400 mb-8">Your unified streaming experience starts here. Add your API keys to load real content from streaming platforms, or add local video files to get started.</p><div className="space-y-4"><button onClick={() => setShowSettings(true)} className="w-full px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Setup API Keys</button><button onClick={handleSelectLocalFolder} className="w-full px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">Add Local Videos</button></div></div></div>}
        </div>
    );
};
...
const StreamingHub = () => {
  ...
  const fetchTMDBContent = useCallback(async (endpoint, platform = 'netflix') => {
      ...
      try {
          ...
          return data.results?.slice(0, 10).map(item => ({ id: `${platform}-${item.id}`, title: item.title || item.name, platform, type: item.media_type || (item.title ? 'movie' : 'series'), rating: item.vote_average?.toFixed(1) || 'N/A', year: new Date(item.release_date || item.first_air_date || '2024').getFullYear(), genre: 'Drama', image: item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : `https://via.placeholder.com/300x450/333/white?text=${encodeURIComponent(item.title || item.name)}`, overview: item.overview, tmdbId: item.id })) || [];
  ...
  const loadRealContent = useCallback(async () => {
      ...
      try {
          ...
          setRealContent({ trending: [...trending, ...youtube.slice(0, 5)], newReleases: popular, top10: topRated, youtube });
      } catch (error) { console.error('Error loading content:', error); }
      setLoading(false);
  }, [tmdbApiKey, youtubeApiKey, fetchTMDBContent, fetchYouTubeContent]);
          ...
          if ('showDirectoryPicker' in window) {
              ...
              for await (const [name, handle] of dirHandle.entries()) {
                  if (handle.kind === 'file' && (name.endsWith('.mp4') || name.endsWith('.mkv') || name.endsWith('.avi') || name.endsWith('.mov'))) {
                      ...
                      files.push({ id: `local-${Date.now()}-${Math.random()}`, title: name.replace(/\.[^/.]+$/, ""), platform: 'local', type: 'video', file, url: URL.createObjectURL(file), duration: 0, size: file.size, lastModified: file.lastModified });
                  }
              }
              setLocalFiles(files);
              localStorage.setItem('localFiles', JSON.stringify(files.map(f => ({ ...f, file: null, url: null }))));
          } else { alert('File System Access API is not supported in this browser. Please use Chrome or Edge.'); }
      } catch (err) { console.error('Error accessing files:', err); }
  }, []);
  
  const updateContinueWatching = useCallback((content, time = 0) => {
      const updated = continueWatching.filter(item => item.id !== content.id);
      updated.unshift({ ...content, lastWatched: Date.now(), currentTime: time });
      const trimmed = updated.slice(0, 10);
      setContinueWatching(trimmed);
      localStorage.setItem('continueWatching', JSON.stringify(trimmed));
      const newStats = { ...viewingStats };
      newStats.totalHours += time / 3600;
      newStats.recentWatches.unshift({ title: content.title, platform: content.platform, watchedAt: Date.now() });
  ...
  }, [continueWatching, viewingStats]);

  const playVideo = useCallback((content) => {
      if (content.platform === 'local') { setCurrentVideo(content); setCurrentView('player'); }
      else if (content.platform === 'youtube' && content.videoId) { window.open(`https://www.youtube.com/watch?v=$${content.videoId}`, '_blank'); updateContinueWatching(content); }
      else { window.open(platformUrls[content.platform](content.title), '_blank'); updateContinueWatching(content); }
  }, [updateContinueWatching, platformUrls]);

  const togglePlayPause = useCallback(() => { if (videoRef.current) { if (isPlaying) videoRef.current.pause(); else videoRef.current.play(); setIsPlaying(!isPlaying); } }, [isPlaying]);
  ...
  const getAllContent = useCallback(() => {
      ...
      return all.filter(item => {
          ...
          const searchMatch = searchQuery === '' || item.title.toLowerCase().includes(searchQuery.toLowerCase()) || (item.genre && item.genre.toLowerCase().includes(searchQuery.toLowerCase())) || (item.channelTitle && item.channelTitle.toLowerCase().includes(searchQuery.toLowerCase()));
          return platformMatch && searchMatch;
      });
  }, [realContent, localFiles, selectedPlatforms, searchQuery]);
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-black' : 'bg-white'} text-white transition-colors duration-300`}>
      <Navigation
        ...
        loadRealContent={loadRealContent}
        loading={loading}
      ...
      <main>
        {currentView === 'home' && (
          <HomeView
            ...
            youtubeApiKey={youtubeApiKey}
            setShowSettings={setShowSettings}
            handleSelectLocalFolder={handleSelectLocalFolder}
